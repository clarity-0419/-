...
public class DatabaseManager
{
    ...
    public async void LoadComboBoxItems(string query, ComboBox comboBox)
    {
        ...
        {
            MessageBox.Show($"搜索信息时发生错误: {ex.Message}");
    ...
    public async Task<List<Employee>> GetEmployees(string searchTerm = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show($"获取信息时发生错误: {ex.Message}");
            return new List<Employee>();
        }
    }
    ...
    public async void InsertEmployee(Employee employee)
    {
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show($"插入记录时发生错误: {ex.Message}");
        }
    }
    ...
    public async void InsertOrUpdateEmployee(List<Employee> employee)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                errorMessage = "插入记录时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "插入记录时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "插入记录时发生错误: " + ex.Message;
            }
    ...
    public async void UpdateEmployeeAndTrainingRecord(Employee employee,string oldId)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex.Message.Contains("唯一约束"))
            {
                errorMessage = "更新信息时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "更新信息时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "更新信息时发生错误: " + ex.Message;
            }
    ...
    public async Task< List<TrainingRecord>> InsertTrainingRecordOrUpdate(List<TrainingRecord> trainingRecord)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex.Message.Contains("唯一约束"))
            {
                errorMessage = "更新记录时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "更新记录时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "更新记录时发生错误: " + ex.Message;
            }
    ...
    public async Task<List<TrainingRecord>> GetTrainingRecordsByEmployeeId(String employeeId="")
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex.Message.Contains("唯一约束"))
            {
                errorMessage = "获取指定记录时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "获取指定记录时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "获取指定记录时发生错误: " + ex.Message;
            }
    ...
    public async Task<List<TrainingRecord>> GetALLTrainingRecords()
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex.Message.Contains("唯一约束"))
            {
                errorMessage = "获取记录时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "获取记录时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "获取记录时发生错误: " + ex.Message;
            }
    ...
    public async Task <List<ImportHistory>> GetImportHistoriesByImportTime(string importTime)
    {
        ...
        {
            MessageBox.Show("根据importTime查询导入记录错误：" + ex.Message);
            return null;
        }
    }
    ...
    public async void InsertOrUpdateImportHistory(List<ImportHistory> importHistory)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex.Message.Contains("唯一约束"))
            {
                errorMessage = "导入记录时发生错误: 违反唯一约束。";
            }
            else if (ex.Message.Contains("字段"))
            {
                errorMessage = "导入记录时发生错误: 使用不当，请检查插入的字段。";
            }
            else
            {
                errorMessage = "导入记录时发生错误: " + ex.Message;
            }
